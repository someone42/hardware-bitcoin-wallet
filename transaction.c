...
static TransactionErrors parseTransactionInternal(BigNum256 sig_hash, BigNum256 transaction_hash, uint32_t length)
{
	...
	for (i = 0; i < num_outputs; i++)
	{
		...
		{
			// Check for a standard, pay to public key hash output script.
		...
		{
			// Check for a standard, pay to script hash output script.
...
0x40, 0x49, 0xff, 0x07, 0x19, 0x3b, 0x7a, 0x02, // previous output
0x17, 0x74, 0xb0, 0x43, 0xc9, 0xe5, 0x44, 0x7a,
0x2c, 0xf3, 0xfa, 0xc6, 0x23, 0x24, 0x47, 0x53,
0xcc, 0xe6, 0xc7, 0x17, 0xd9, 0x83, 0x65, 0x94,
...
0xFF, 0xFF, 0xFF, 0xFF, // sequence
0xf2, 0x6d, 0xf8, 0x7e, 0xab, 0xea, 0x87, 0xad, // previous output
0xee, 0xf8, 0xf7, 0x86, 0x54, 0x81, 0x59, 0x0c,
0xd9, 0xf4, 0x41, 0x40, 0x0d, 0x2a, 0xcd, 0x03,
0xe2, 0x37, 0xf1, 0xbc, 0x5a, 0xcd, 0x8b, 0x78,
0x00, 0x00, 0x00, 0x00, // number in previous output
0x6A, // script length
...
int main(void)
{
	...
	testTransaction(good_p2sh_transaction, sizeof(good_p2sh_transaction), "good_p2sh", TRANSACTION_NO_ERROR);
	...
	testTransaction(generated_transaction, sizeof(good_p2sh_transaction), "bad_p2sh1", TRANSACTION_NON_STANDARD);
	memcpy(generated_transaction, good_p2sh_transaction, sizeof(good_p2sh_transaction));
	generated_transaction[345] = 0x15;
	testTransaction(generated_transaction, sizeof(good_p2sh_transaction), "bad_p2sh2", TRANSACTION_NON_STANDARD);
	memcpy(generated_transaction, good_p2sh_transaction, sizeof(good_p2sh_transaction));
	generated_transaction[366] = 0x88;
	testTransaction(generated_transaction, sizeof(good_p2sh_transaction), "bad_p2sh3", TRANSACTION_NON_STANDARD);
	free(generated_transaction);
	testTransaction(nonstandard_p2sh_transaction1, sizeof(nonstandard_p2sh_transaction1), "nonstandard_p2sh_transaction1", TRANSACTION_NON_STANDARD);
	testTransaction(nonstandard_p2sh_transaction2, sizeof(nonstandard_p2sh_transaction2), "nonstandard_p2sh_transaction2", TRANSACTION_NON_STANDARD);
	testTransaction(nonstandard_p2sh_transaction3, sizeof(nonstandard_p2sh_transaction3), "nonstandard_p2sh_transaction3", TRANSACTION_NON_STANDARD);
	testTransaction(nonstandard_p2sh_transaction4, sizeof(nonstandard_p2sh_transaction4), "nonstandard_p2sh_transaction4", TRANSACTION_NON_STANDARD);
	...
	for (i = 0; i < sizeof(good_p2sh_transaction); i++)
	{
		...
		testTransaction(good_p2sh_transaction, (uint32_t)i, name, TRANSACTION_INVALID_FORMAT);
	}
